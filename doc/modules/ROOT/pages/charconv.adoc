...
to_chars(char* first, char* last, const basic_big_int<b, LimbType, A>& value, int base = 10);

template <std::size_t b, class LimbType, class A>
constexpr std::from_chars_result
from_chars(const char* first, const char* last, basic_big_int<b, LimbType, A>& value, int base = 10);
