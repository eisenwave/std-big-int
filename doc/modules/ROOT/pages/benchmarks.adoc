...
Dedicated processor architecture-specific optimizations using hand-written
...
processor's assembly dialect. The simple optimization of long multiplication
...
splitting-algorithms Karatsuba and all orders of Toom-Cook are optimized.
This is because after the splitting is performed, the real work of these
algorithms still falls through to basecase long multiplication.
Simultaneously sub-quadratic large-limb-count long division and base conversion
are optimized since they are directly built atop the multiplication algorithms
and  directly benefit from their optimizations.

Experience shows, with one single assembly file that performs basecase
long multiplication, significant improvements in the performance of
nearly the entire library can be achieved.

One proof of concept is included for the popular `x86_64` architecture
in its generic form. We have implemented basecase long multiplication
in hand-coded assembly for `x86_64` on compiler systems GCC and clang,
the detections of which happen at compile time.

Results are shown in the tables below using an 11th Gen Intel(R) Core(TM) i9-11900K
at 3.50GHz, 3401 Mhz, 8 Core(s), 16 Logical Processor(s), running Ubuntu 24.04
within WSL2 compiled/assembled with GCC-13. Comparison of multiplication operations
over a wide range of limb counts with and without assembly optimization
are have been compared with the same calculations for GMP. The heats
ran 131,072 trials for multiplication with limb counts ranging from 16-2,048
...
|===
|  long mul impl  | `beman::big_int` [us-per-mul] | `boost::gmp_int` [us-per-mul] |  ratio   | rel improve [percent]

| C++             |  305   |  95[1]   |  3.2  | --- |
| hand-coded asm  |  210   |  95[1]   |  2.2  | 32  |
|===

The relative performance improvement is almost one third, simply via creation and
inclusion of a single hand-coded assembly routine for long multiplication.

An adaption of the program
https://github.com/eisenwave/std-big-int/blob/main/tests/beman/big_int/perf/mul_big_int_vs_gmp_cpp.limbs.cpp[mul_big_int_vs_gmp_cpp.limbs.cpp]
has been used for this local result.
