...
* *128-bit tier* -- compares `__int128` with `basic_big_int<128>` (two inplace limbs),
...
factor, and for division the two-limb path of `big_int` (11.6 ns) is in the same
...
This is often the case for both the reference application as well as for potential
...
Experience shows that with one single assembly file performing basecase
...
Results for multiplication are explicitly shown in the table below.
Similar performance improvements can be observed for division.
For this measurement, an Intel(R) Core(TM) i9-11900K at 3.50GHz has been used.
...
The routine handles `x * x` from 5 limbs up to the Karatsuba squaring
threshold and the leaves of the Karatsuba squaring recursion, so the
Toom-Cook squaring tiers built on top of them speed up too. Below 5 limbs,
...
pass:[C++] version of the same algorithm takes its place.

The table below compares the new routine with the two basecases that squares
used before it: the portable pass:[C++] `square_long`, and the general
assembly multiplication called with `a * a` (used for fewer than 8 limbs).
Times are per square, measured on the same Intel(R) Core(TM) i9-11900K.

[cols="1,1,1,1", options="header"]
|===
| limbs | pass:[C++] `square_long` [ns] | ASM multiply `a * a` [ns] | ASM square [ns]
...
https://github.com/eisenwave/std-big-int/blob/main/src/square_long_runtime_x86_64_generic.s[square_long_runtime_x86_64_generic.s]
and
https://github.com/eisenwave/std-big-int/blob/main/src/square_long_runtime_x86_64_generic.asm[square_long_runtime_x86_64_generic.asm].

[#benchmarks_arch_opts_aarch64]
=== AArch64
...
The table below compares the portable pass:[C++] `multiply_long` against the
assembly `multiply_long_runtime`, measured per operation on an Apple M4 Max
(`appleclang-release`).

[cols="1,1,1", options="header"]
|===
| limbs | pass:[C++] `multiply_long` [ns] | ASM `multiply_long_runtime` [ns]
...
single pass. The table below compares the portable pass:[C++] `square_long`,
...
The implementations can be found in
https://github.com/eisenwave/std-big-int/blob/main/src/multiply_long_runtime_aarch64_generic.S[multiply_long_runtime_aarch64_generic.S],
https://github.com/eisenwave/std-big-int/blob/main/src/multiply_long_runtime_aarch64_generic.asm[multiply_long_runtime_aarch64_generic.asm],
https://github.com/eisenwave/std-big-int/blob/main/src/square_long_runtime_aarch64_generic.S[square_long_runtime_aarch64_generic.S],
and
https://github.com/eisenwave/std-big-int/blob/main/src/square_long_runtime_aarch64_generic.asm[square_long_runtime_aarch64_generic.asm].
