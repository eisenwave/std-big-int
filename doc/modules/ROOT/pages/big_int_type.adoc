...
`std::length_error`. The check happens before the allocator is consulted, so no
allocation is attempted for a request that cannot be represented.
A request that is within the limit but cannot be served reports the allocator's own
failure (typically `std::bad_alloc`) instead.
During constant evaluation the same condition is a
compile-time error rather than an exception, since a thrown exception is not a
constant expression.
