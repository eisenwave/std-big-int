...
* *Preconditions:* `s >= 0`, and `s` does not exceed the maximum shift `max_size() * std::numeric_limits<uint_multiprecision_t>::digits`.
* *Postconditions:* None.
...
Returns the maximum number of limbs that the representation may contain.
...
Returns the maximum number of limbs that can be part of the representation. This
is the same limit reported by `max_size()`.
...
A directive that informs the object of a planned increase in size so that
storage can be allocated up front. Reallocation occurs if and only if `n` is
greater than the current effective capacity (`inplace_capacity` while the value
is stored in place, otherwise `capacity()`).
...
Returns the number of limbs of dynamically-allocated storage currently
available, or `0` when the value is held in the in-place (small-object) storage.
...
currently available. Unlike `capacity()`, which reports `0` while the value is
held in place, this never reports fewer than `inplace_capacity` limbs.
...
* *Preconditions:* `s >= 0`, and `s` does not exceed the maximum shift for the result type (`max_size() * std::numeric_limits<uint_multiprecision_t>::digits`).
* *Postconditions:* None.
