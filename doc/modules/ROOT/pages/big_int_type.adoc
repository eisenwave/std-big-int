...
| `inplace_capacity`
| The number of `Limb` limbs available in the in-place (small-object) storage. It is at least large enough to hold `min_inplace_bits` bits, and never smaller than what fits in the object's pointer footprint.

| `inplace_bits`
| `inplace_capacity * std::numeric_limits<Limb>::digits`; the number of bits of in-place storage. A value whose magnitude fits in `inplace_bits` bits is stored without allocating.
|===
...
`allocator_traits<Allocator>::select_on_container_copy_construction(x.get_allocator())`.
...
Initializes the integer value to that of `x` and initializes the allocator to
`a`. Unlike the copy constructor, `select_on_container_copy_construction` is
not consulted, so this is how a copy is placed on a chosen allocator, such as
the source's own: `basic_big_int y(x, x.get_allocator());`.
...
Initializes the integer value to that of `x` and initializes the allocator to
`a`. If `allocator_traits<Allocator>::is_always_equal::value` is `true` or
`a == x.get_allocator()`, the storage of `x` is taken over as by the move
constructor. Otherwise `a` cannot deallocate that storage, so the value is
copied into storage obtained from `a`, as by the allocator-extended copy
constructor.

* *Preconditions:* None.
* *Postconditions:* `x` is left in a valid state with an unspecified value.
* *Throws:* Nothing if the storage of `x` is taken over or the effective width of the integer value of `x` is less than or equal to `inplace_bits`; otherwise, any exception thrown during allocation. If an exception is thrown, `x` is unchanged.

NOTE: Uses-allocator construction calls these two constructors, so an allocator-aware
container such as `std::pmr::vector` copies or moves each `basic_big_int` element onto
its own allocator. When the element already uses that allocator, as it does when the
container reallocates, the move takes over the element's storage instead of allocating.
Because the allocator parameter is a non-deduced context, class template argument
deduction takes the type from `x` alone, and `a` may be anything convertible to
`Allocator`: `basic_big_int y(x, &resource)` deduces the same `pmr::basic_big_int`
as `x`.
...
* *Preconditions:* None.
* *Postconditions:* None.
* *Throws:* `std::length_error` if the result would occupy more than `max_size()` bits; otherwise, any exception thrown during allocation.
...
* *Preconditions:* `std::allocator_traits<A>::propagate_on_container_swap::value` is `true`, or `x.get_allocator() == y.get_allocator()`.
* *Postconditions:* `x` holds the integer value that `y` held before the call, and `y` holds the integer value that `x` held before the call.
* *Throws:* Nothing.
...
`std::length_error`; see xref:big_int_type.adoc#big_int_type_length_limit[the length limit].

This is a ceiling on the representation, not a largest value of the type, and it
lies far beyond any allocation a program can make. `std::numeric_limits`
accordingly reports `basic_big_int` as unbounded; see
xref:limits.adoc#limits_bounded[Why `is_bounded` is `false`].
...
* *Throws:* `std::length_error` if `n` is greater than `max_size()`; otherwise, any exception thrown during allocation. In either case `*this` is unchanged.
...
* *Throws:* `std::length_error` if `n` is greater than `max_representation_size()`; otherwise, any exception thrown during allocation. In either case `*this` is unchanged.
...
The `const&` overloads build their result by copy construction, so its
allocator is
`allocator_traits<Allocator>::select_on_container_copy_construction(get_allocator())`,
as `abs`'s is. The `&&` overloads take over the operand's storage and so carry
the operand's own allocator.
...
Equivalent to `return *this;` for the lvalue overload and
`return std::move(*this);` for the rvalue overload, so each selects the
corresponding constructor and the allocator behavior above follows from it. The
value is unchanged.
...
Neither is ref-qualified, so both apply to an rvalue.
...
Every operator in this section returns a new value, so its allocator is the
`basic_big_int` operand's allocator run through
...
negative infinity, where `v` is the integer value of `x`. The result's allocator
is selected as described for the binary operators above.
...
xref:big_int_type.adoc#big_int_type_unit_multi_t[`uint_multiprecision_t`] on the
