...
* A pass:[C++] compiler that conforms to the pass:[C++23] standard or greater
* CMake 3.30 or later
...
|===
| Compiler | Version | pass:[C++] Standards | Standard Library

| GCC | >= 11 | >= pass:[C++23] | pass:[libstdc++]
| Clang | >= 17 | >= pass:[C++23] | pass:[libstdc++], pass:[libc++]
| AppleClang | latest | >= pass:[C++23] | pass:[libc++]
...
[#build_and_usage_modules]
=== Optional: the pass:[C++] named module
...
This needs a newer toolchain than the header build, and is not available with
AppleClang. See xref:modules.adoc[pass:[C++] Named Modules] for the full
story.
...
To use `beman.big_int` in your pass:[C++] project, include an appropriate
...
[TIP]
====
To consume `beman.big_int` as a named module instead, `import beman.big_int;`
and link `beman::big_int_module` in place of `beman::big_int`. See
xref:modules.adoc[pass:[C++] Named Modules].
====
