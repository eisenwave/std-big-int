...
static auto do_one_test() -> bool {
    ...
    using bn_type = beman::big_int::basic_big_int<std::numeric_limits<typename allocator_type::value_type>::digits,
                                                  allocator_type>;

    const bn_type val_a{};
    const bn_type val_b{};
    const bn_type val_ctrl{};

    static_cast<void>(from_chars(pstr_a,    pstr_a    + sizeof(pstr_a),    val_a, 16));
