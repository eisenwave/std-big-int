...
std::ostream& operator<<(std::ostream& out, const basic_big_int<b, LimbType, A>& x) {
...
template <std::size_t b, class LimbType, class A>
[[nodiscard]] constexpr bool is_inplace(const basic_big_int<b, LimbType, A>& x) noexcept {
    return x.representation_capacity() == basic_big_int<b, LimbType, A>::inplace_capacity;
}
