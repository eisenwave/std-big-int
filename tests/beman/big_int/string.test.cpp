...
// These tests mirror the to_string coverage in charconv.test.cpp for the
// wide-character to_wstring overload. to_wstring renders through the same
// to_chars path and then widens the narrow ASCII output, so each fixed-value
// check below is the L"..." twin of its to_string counterpart.
...
[[nodiscard]] std::string random_digit_string(const std::size_t len, const int base, const std::uint64_t seed) {
    ...
    if (s.front() == '0') {
        s.front() = alphabet[1]; // no leading zero, so to_wstring round-trips the string exactly
    }
    return s;
}
...
// Large-input coverage for the sub-quadratic to_chars path, mirroring the
// to_string round-trips: the wide output must match the widened narrow input
// across both the non-power-of-two fast kernel and the power-of-two bit-packing.
TEST(WCharconv, FastPathRoundTrip) {
