...
// %GCC% -std=c++20 -Wall -Wextra -O3 -IC:/ChrisGitRepos/std-big-int/include -IC:/boost/boost_1_90_0 ./test.cpp -o
// test.exe
// .\test.exe
...
template <typename IntegralTimePointType, typename ClockType = std::chrono::high_resolution_clock>
...
constexpr std::streamsize limb_chars { static_cast<std::streamsize>(std::numeric_limits<beman::big_int::uint_multiprecision_t>::digits / 4) };
constexpr unsigned        limb_width { static_cast<unsigned>(std::numeric_limits<beman::big_int::uint_multiprecision_t>::digits) };
...
auto main() -> int
{
  ...
  for( ; ((index < trials) && result_is_ok); ++index)
  {
    ...
    if((++print_count_prescaler % 1024U) == 0U)
    {
